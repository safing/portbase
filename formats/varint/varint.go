...
func Unpack8(blob []byte) (uint8, int, error) {
	...
	if blob[1] != 0x01 {
		return 0, 0, &valueExceededError{max: "255 (uint8)"}
	}
	return blob[0], 1, nil
}
...
func Unpack16(blob []byte) (uint16, int, error) {
	...
	if r < 0 {
		return 0, 0, &valueExceededError{max: "18446744073709551615 (uint64)"}
	}
	if n > 65535 {
		return 0, 0, &valueExceededError{max: "65535 (uint16)"}
	}
...
func Unpack32(blob []byte) (uint32, int, error) {
	...
	if n > 4294967295 {
		return 0, 0, &valueExceededError{max: "4294967295 (uint32)"}
	}
